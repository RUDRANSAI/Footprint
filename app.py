...
def process_image_data(image_bytes, contact_width_mm, threshold_percent, tyre_name):
    ...
    try:
        ...
        percent_black = (number_of_black_pix / sum_pix)
        ...
        arr2D_TL_Flp_Fill = np.flipud(painter(deepcopy(arr2D_TL_Act_Fill)))
        arr2D_TR_Flp_Fill = np.flipud(painter(deepcopy(arr2D_TR_Act_Fill)))
        arr2D_BL_Flp_Fill = np.flipud(painter(deepcopy(arr2D_BL_Act_Fill)))
        arr2D_BR_Flp_Fill = np.flipud(painter(deepcopy(arr2D_BR_Act_Fill)))
        ...
        black_initial = np.count_nonzero(np.array(bnw_image) == 0)
...
if process_button:
    if uploaded_file is not None:
        ...
        with st.spinner('Analyzing image...'):
            # Call the processing function and store the result in session state
